...
@FXML
 ...
 public void login(ActionEvent event) throws IOException {
   ...
   FXMLLoader loader = new FXMLLoader(getClass().getResource("displayData.fxml"));
    ...
    if ((!appnameText.getText().matches("[a-zA-Z]*"))||!appageText.getText().matches(("\\d*")) || !immnameText.getText().matches("[a-zA-Z]*") || !immageText.getText().matches("\\d*"))
    {
      Alert alert2 = new Alert(AlertType.ERROR);
      alert2.setTitle("Invalid fields!");
      alert2.setHeaderText("One of your fields has an invalid string type!");
      alert2.setContentText("Cannot continue. Field has invalid string!");
      alert2.show();
    }
    else if(appnameText.getText().isEmpty()||appaddressText.getText().isEmpty()||appage==0||appid == 0 || appemailText.getText().isEmpty() || immnameText.getText().isEmpty()|| immaddressText.getText().isEmpty()|| immage == 0 || immid == 0)
    {
      //alert
      Alert alert = new Alert(AlertType.ERROR);
      alert.setTitle("Empty fields!");
    ...
    else
    {
      ...
      Document doc = Document.createDoc(immigrant, applicant, 400, 999);
      if(Workflow.createWorkflow(doc))
      {
        if(doc==null)
        {
          System.out.println("UH OHHHHHHHHHHHHHHHHHH");
        }
        else
        {
          System.out.println("Applicant:" + Workflow.getDoc().getApplicant().getAddress() + "!");
        }
